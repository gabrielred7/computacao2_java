...
package otimizandoarmarios;

import java.util.Scanner;

public class OtimizandoArmarios {
    ...
    public static int contarArmariosAbertos2(int contArmarios) {
        ...
        for (int crianca = 1; crianca <= contArmarios; crianca++) {
            for (int indice = crianca; indice <= contArmarios; indice += crianca) {
                if (indice % crianca == 0) {  // eh multiplo!
                    armarios[indice] = !armarios[indice];
                }
            }
    ...
    public static void main(String[] args) {
        ...
        resultado = contarArmariosAbertos((int) quantidadeDeArmarios);
        duracaoEmMillis = System.currentTimeMillis() - inicio;
        duracaoEmSegundos = duracaoEmMillis / (float) 1000;
        System.out.println(String.format("(1) Quant armários abertos = %d (tempo: %.3f segundos)", resultado, duracaoEmSegundos));

       inicio = System.currentTimeMillis();
        resultado = contarArmariosAbertos2((int) quantidadeDeArmarios);
        duracaoEmMillis = System.currentTimeMillis() - inicio;
        duracaoEmSegundos = duracaoEmMillis / (float) 1000;
        System.out.println(String.format("(2) Quant armários abertos = %d (tempo: %.3f segundos)",resultado, duracaoEmSegundos));

        inicio = System.currentTimeMillis();
        resultado = contarArmariosAbertos3((int) quantidadeDeArmarios);
        duracaoEmMillis = System.currentTimeMillis() - inicio;
        duracaoEmSegundos = duracaoEmMillis / (float) 1000;
        System.out.println(String.format("(3) Quant armários abertos = %d (tempo: %.3f segundos)",  resultado, duracaoEmSegundos));

        inicio = System.currentTimeMillis();
        resultado = contarArmariosAbertos5(quantidadeDeArmarios);
        duracaoEmMillis = System.currentTimeMillis() - inicio;
        duracaoEmSegundos = duracaoEmMillis / (float) 1000;
        System.out.println(String.format("(5) Quant armários abertos = %d (tempo: %.3f segundos)", resultado, duracaoEmSegundos));
    }
  }